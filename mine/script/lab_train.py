...
def train(option):
    for i in range(len(option.weight_list)):
        ...
        logging.basicConfig(level=logging.INFO, filename=(os.path.join(option.single_test_folders[i], "train_running.log")))
        ...
        try:
            ...
            for e in range(EPOCHOS):
                ...
                save_model_path = os.path.join(os.path.join(option.single_test_folders[i], "model_save_dir"), save_model_filename)
                torch.save(style_model.state_dict(), save_model_path)
